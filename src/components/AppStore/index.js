...
class AppStore extends Component {
  state = {inputSearch: '', activetedTabId: tabsList[0].tabId}
  ...
  onTabChange = tabId => {
    this.setState({activetedTabId: tabId})
  }

  tabAppList = () => {
    const {activetedTabId} = this.state
    const currentAppsList = appsList.filter(
      each => each.category === activetedTabId,
    )
  ...
  render() {
    const {inputSearch, activetedTabId} = this.state
    const currentTabAppsList = this.tabAppList()
    const filterdItemsList = currentTabAppsList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(inputSearch.toLocaleLowerCase()),
    )
        ...
        <ul className="tabItems">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              isTabActive={activetedTabId === eachTab.tabId}
            />
          ))}
        </ul>

        <ul className="app-item-container">
          {filterdItemsList.map(eachItem => (
            <AppItem itemDetails={eachItem} key={eachItem.appId} />
          ))}
        </ul>
      </div>
    )
